...
|---|---|---|
| TimeM | Минуты таймера | Пишет Unity (SendTimeMToJS), читает ui.js |
| TimeS | Секунды таймера | Пишет Unity (SendTimeSToJS), читает ui.js |
| Time: | Двоеточие | Не используется в коде |
| Step | Шаги | Пишет Unity (SendStepToJS), читает ui.js |
| Speed | Скорость | Пишет Unity (SendSpeedToJS), также пишет JS при движении слайдера |
| unity-container / unity-canvas / unity-loading-bar / unity-logo / unity-progress-bar-empty / unity-progress-bar-full | Область и загрузчик Unity | main.js |
| unity-warning | Баннеры предупреждений | main.js |
| unity-footer / unity-webgl-logo / unity-fullscreen-button / unity-build-title | Футер/фуллскрин | main.js |
| diagnostics-icon | Иконка диагностики | main.js |
| solve-button / stats-button | Кнопки действий | main.js/unity.js → SendMessage('Cube', ...) |
| save-button / load-button / export-stats-button | Кнопки сохранения/загрузки | save.js/main.js → SendMessage('SaveSystem', ...) |
| speed-slider | Слайдер скорости | unity.js/main.js → WebGL_SetSpeed |
| pastel-button / standard-button / neon-button / grey-button | Стили куба | ui.js → CreateCube(style) |
| warning-modal / win-modal / stats-modal | Модалки | ui.js управляет display |
| win-time / win-steps / win-speed | Поля модалки победы | Заполняет ui.js |
| Date | Дата в модалке победы | Сейчас не заполняется (логика закомментирована) |
| stats-content | Текст модалки статистики | ui.js пишет “Загрузка…” |
...
|---|---|---|---|
| CubeManager.SendTime/SendNumberInt/SendNumberFloat | DllImport("__Internal") | WebGLSay.jslib → document.getElementById(...).innerText | Обновление TimeM/TimeS/Step/Speed |
...
| SaveSystem.SaveToJSON (WebGL) | DllImport("__Internal") DownloadFile | SaveSystem.jslib | Скачать JSON через Blob |
| SaveSystemBridge.WebGL_LoadGame | DllImport("__Internal") TriggerFileUpload | SaveSystem.jslib | Открыть file picker |
